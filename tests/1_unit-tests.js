...
suite('Unit Tests', () => {
    suite('American to British', () => {
        arrayAtB.forEach(d => {
            test(d[0], done => {
                assert.equal(translator.americanToBritish(d[0]), d[1]);
                done();
            });
        });
    });
    suite('British to American', () => {
        arrayBtA.forEach(d => {
            test(d[0], done => {
                assert.equal(translator.britishToAmerican(d[0]), d[1]);
                done();
            });
        });
    });
    suite('Highlight', () => {
        ...
        test('Paracetamol takes up to an hour to work.', done => {
            assert.equal(translator.highlighter('Paracetamol takes up to an hour to work.',
